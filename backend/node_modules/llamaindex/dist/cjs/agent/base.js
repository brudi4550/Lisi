...
async function* createTaskImpl(handler, context, _input) {
    let isFirst = true;
    let isDone = false;
    let input = _input;
    let prevStep = null;
    while(!isDone){
        ...
        if (isFirst) {
            (0, _CallbackManager.getCallbackManager)().dispatchEvent("agent-start", {
                payload: {
                    startStep: step
                }
            });
        ...
        if (!isLast) {
            if (output) {
                input = (0, _utils.isAsyncIterable)(output) ? await (0, _utils2.consumeAsyncIterable)(output) : output.message;
            } else {
                input = null;
            }
        }
        ...
        if (isLast) {
            isDone = true;
            (0, _CallbackManager.getCallbackManager)().dispatchEvent("agent-end", {
                payload: {
                    endStep: step
                }
            });
...
class AgentRunner {
    ...
    async createTask(message, stream = false) {
        ...
        return this.#runner.createTask((0, _utils1.extractText)(message), {
            stream,
            toolCallCount: 0,
            llm: this.#llm,
    ...
    async chat(params) {
        ...
        const stepOutput = await (0, _env.pipeline)(task, async (iter)=>{
